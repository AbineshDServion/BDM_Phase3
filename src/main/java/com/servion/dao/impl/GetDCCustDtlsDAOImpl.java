...
import com.bankmuscat.esb.cardmanagementservice.GetDCCustDtlsResType;
import com.servion.common.util.LoggerObject;
import com.servion.common.util.WriteLog;
import com.servion.common.util.util;
import com.servion.dao.GetDCCustDtlsDAO;
...
import com.servion.ws.service.GetDCCustDtlsService;
import com.servion.ws.util.DAOLayerUtils;

public class GetDCCustDtlsDAOImpl implements GetDCCustDtlsDAO{
	private static Logger logger = LoggerObject.getLogger();

	@Autowired
	GetDCCustDtlsService getDCCustDtls;


	

	public GetDCCustDtlsService getGetDCCustDtls() {
		return getDCCustDtls;
	}

	public void setGetDCCustDtls(GetDCCustDtlsService getDCCustDtls) {
		this.getDCCustDtls = getDCCustDtls;
	}
	...
	@Override
	public GetDCCustDtls_HostRes getGetDCCustDtlsHostRes(CallInfo callInfo, String cardNo,String requestType) throws DaoException {
		...
		GetDCCustDtls_HostRes beanResponse = new GetDCCustDtls_HostRes();
		...
		try{
			...
			GetDCCustDtlsResType response = null;

			if(logger.isDebugEnabled()){WriteLog.write(WriteLog.DEBUG,sessionID, "Calling the DAO Layer method callgetGetDCCustDtls host");}
			...
			response = getDCCustDtls.callGetDCCustDtlsHost(logger, sessionID, cardNo, requestType, str_UUI, generateXML, callInfo);
			...
			if(logger.isDebugEnabled()){WriteLog.write(WriteLog.DEBUG,sessionID, "The Host error code of callgetGetDCCustDtls is : "+code);}
			...
			if(!util.isNullOrEmpty(ivr_ICEFeatureData.getConfig().getParamValue(Constants.CUI_CI_GetDCCustDtls_Succ_ErrorCode))){
				if(logger.isDebugEnabled()){WriteLog.write(WriteLog.DEBUG,sessionID, "Success error code received from the Feature ");}
				hostErrorCodeList = (String)ivr_ICEFeatureData.getConfig().getParamValue(Constants.CUI_CI_GetDCCustDtls_Succ_ErrorCode); 
			}
			else{
				if(logger.isDebugEnabled()){WriteLog.write(WriteLog.DEBUG,sessionID, "Success error code received from the Global ");}
				hostErrorCodeList = (String)ivr_ICEGlobalConfig.getConfig().getParamValue(Constants.CUI_CI_GetDCCustDtls_Succ_ErrorCode);
			}
			...
			if(Constants.WS_SUCCESS_CODE.equals(code)){
				if(!util.isNullOrEmpty(response)){
					...
					beanResponse.setMobileNumber(response.getCardDtlsResp().getMobileNumber());
					beanResponse.setEmail(response.getCardDtlsResp().getEmailID());
					
				}else{
					if(logger.isDebugEnabled()){WriteLog.write(WriteLog.DEBUG,sessionID, " ## GetDCCustDtls Response field Received null / empty so setting error code as 1");}
			...
			if(logger.isDebugEnabled()){WriteLog.writeError(WriteLog.ERROR, pe,  "There was an error at GetDCCustDtls.getGetDCCustDtlsHostRes() "	+ pe.getMessage());}
			throw new DaoException(pe);
		}catch(Exception e){
			if(logger.isDebugEnabled()){WriteLog.writeError(WriteLog.ERROR, e,  "There was an error at GetDCCustDtls.getGetDCCustDtlsHostRes() "	+ e.getMessage());}
