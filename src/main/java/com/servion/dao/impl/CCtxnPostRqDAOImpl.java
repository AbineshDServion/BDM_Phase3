...
import com.bankmuscat.esb.financialtxnservice.CCTxnPostResType;
...
import com.servion.ws.service.CCtxnPostRqService;
import com.servion.ws.util.DAOLayerUtils;

public class CCtxnPostRqDAOImpl implements CCtxnPostRqDAO{
	private static Logger logger = LoggerObject.getLogger();
	
	@Autowired
	CCtxnPostRqService cctxnPostRqService;

	
	public CCtxnPostRqService getCctxnPostRqService() {
		return cctxnPostRqService;
	}

	public void setCctxnPostRqService(CCtxnPostRqService cctxnPostRqService) {
		this.cctxnPostRqService = cctxnPostRqService;
	}
	...
	@Override
	public UpdateCreditCardPaymenTxnPosttDetails_HostRes getCCPaymentUpdateHostRes(
			CallInfo callInfo, String reference, String postTo, String acctId,
			String cardNo, XMLGregorianCalendar trxnPostingDate,
			XMLGregorianCalendar trxnDate, BigDecimal origCcyAmt,
			String origCcyCode, String description, String requestType) throws DaoException {
		...
		//if(logger.isInfoEnabled()){WriteLog.write(WriteLog.INFO, "*** ENTER - DISPACHING WS RES ***: CCtxnPostRqDAOImpl.UpdateCreditCardPaymenTxnPosttDetails_HostRes()");}

		UpdateCreditCardPaymenTxnPosttDetails_HostRes beanResponse = new UpdateCreditCardPaymenTxnPosttDetails_HostRes();
				...
				try{
					...
					CCTxnPostResType response = null;

					if(logger.isDebugEnabled()){WriteLog.write(WriteLog.DEBUG,sessionID, "Calling the DAO Layer method callCCTxnUpdHost host");}
					...
					response = cctxnPostRqService.callCCTxnUpdHost(logger, sessionID, reference, postTo, acctId, cardNo, trxnPostingDate, trxnDate, origCcyAmt, origCcyCode, description, requestType, str_UUI, generateXML, callInfo);
					...
					if(logger.isDebugEnabled()){WriteLog.write(WriteLog.DEBUG,sessionID, "The Host error code of callCCTxnUpdHost is : "+code);}
					...
					if(logger.isDebugEnabled()){WriteLog.write(WriteLog.DEBUG,sessionID, "The Host error Short description for getCCPaymentUpdateHostRes is : "+ws_ResponseHeader.getEsbErrDesc());}

					if(logger.isDebugEnabled()){WriteLog.write(WriteLog.DEBUG,sessionID,"### CCTxnPost HOST RESPONSE DETAILS ###" +"CALL ID = "+str_UUI+"### RESPONSE CODE = "+beanResponse.getHostResponseCode() + "### RESPONSE DESC = " + beanResponse.getErrorDesc());}
					...
					if(!util.isNullOrEmpty(ivr_ICEFeatureData.getConfig().getParamValue(Constants.CUI_CI_CCTransactionPost_Succ_ErrorCode))){
						if(logger.isDebugEnabled()){WriteLog.write(WriteLog.DEBUG,sessionID, "Success error code received from the Feature ");}
						hostErrorCodeList = (String)ivr_ICEFeatureData.getConfig().getParamValue(Constants.CUI_CI_CCTransactionPost_Succ_ErrorCode); 
					}
					else{
						if(logger.isDebugEnabled()){WriteLog.write(WriteLog.DEBUG,sessionID, "Success error code received from the Global ");}
						hostErrorCodeList = (String)ivr_ICEGlobalConfig.getConfig().getParamValue(Constants.CUI_CI_CCTransactionPost_Succ_ErrorCode);
					}
					...
					if(Constants.WS_SUCCESS_CODE.equals(code)){
						if(!util.isNullOrEmpty(response)){
							...
							if(logger.isDebugEnabled()){WriteLog.write(WriteLog.DEBUG,sessionID, " ## Response field ## Transaction ID is "+ response.getTransactionID());}
							beanResponse.setTransactionID(response.getTransactionID());
						}
					}else{
						if(logger.isDebugEnabled()){WriteLog.write(WriteLog.DEBUG,sessionID, " ## CCTxnPostRes Service Response field Received null / empty so setting error code as 1");}
					...
					if(logger.isDebugEnabled()){WriteLog.writeError(WriteLog.ERROR, pe,  "There was an error at CCtxnPostRqDAOImpl.UpdateCreditCardPaymenTxnPosttDetails_HostRes() "	+ pe.getMessage());}
					throw new DaoException(pe);
				}catch(Exception e){
					if(logger.isDebugEnabled()){WriteLog.writeError(WriteLog.ERROR, e,  "There was an error at CCtxnPostRqDAOImpl.UpdateCreditCardPaymenTxnPosttDetails_HostRes() "	+ e.getMessage());}
					throw new DaoException(e);
				}

		//if(logger.isInfoEnabled()){WriteLog.write(WriteLog.INFO, "*** EXIT - DISPACHING WS RES ***: CCtxnPostRqDAOImpl.UpdateCreditCardPaymenTxnPosttDetails_HostRes()");}
		return beanResponse;
	}
