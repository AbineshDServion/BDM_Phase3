...
import com.bankmuscat.esb.financialtxnservice.XferFundResType;
...
import com.servion.ws.service.FundTransferIntraService;
import com.servion.ws.util.DAOLayerUtils;

public class FundTransferIntraDAOImpl implements FundTransferIntraDAO{
	private static Logger logger = LoggerObject.getLogger();

	@Autowired
	FundTransferIntraService fundTransferIntraService;

	

	public FundTransferIntraService getFundTransferIntraService() {
		return fundTransferIntraService;
	}

	public void setFundTransferIntraService(
			FundTransferIntraService fundTransferIntraService) {
		this.fundTransferIntraService = fundTransferIntraService;
	}
	...
	@Override
	public UpdateFTIntraPayment_HostRes getFTInterPaymentUpdHostRes(
			CallInfo callInfo, String xferId, String benfCode,
			BigDecimal debitAmt, BigDecimal creditAmt, String acctID,
			String destAcctID, String serviceProviderID, String utilityID,
			String billID, String contractID, String requestType) throws DaoException {
		...
		//if(logger.isInfoEnabled()){WriteLog.write(WriteLog.INFO, "*** ENTER - DISPACHING WS RES ***:FundTransferIntraDAOImpl.getFTInterPaymentUpdHostRes()");}

		UpdateFTIntraPayment_HostRes beanResponse = new UpdateFTIntraPayment_HostRes();
		...
		try{
			...
			XferFundResType response = null;

			if(logger.isDebugEnabled()){WriteLog.write(WriteLog.DEBUG,sessionID, "Calling the DAO Layer method callFundsTransferIntraHost host");}
			...
			response = fundTransferIntraService.callFundsTransferIntraHost(logger, sessionID, xferId, benfCode, debitAmt, creditAmt, acctID,
					destAcctID, serviceProviderID, utilityID, billID, contractID, requestType, str_UUI, generateXML, callInfo);
			...
			if(logger.isDebugEnabled()){WriteLog.write(WriteLog.DEBUG,sessionID, "The Host error code of callFundsTransferIntraHost is : "+code);}
			...
			if(logger.isDebugEnabled()){WriteLog.write(WriteLog.DEBUG,sessionID, "The Host error Short description for getFTInterPaymentUpdHostRes is : "+ws_ResponseHeader.getEsbErrDesc());}

			if(logger.isDebugEnabled()){WriteLog.write(WriteLog.DEBUG,sessionID,"### FundTransferIntra HOST RESPONSE DETAILS ###" +"CALL ID = "+str_UUI+"### RESPONSE CODE = "+beanResponse.getHostResponseCode() + "### RESPONSE DESC = " + beanResponse.getErrorDesc());}
			...
			if(!util.isNullOrEmpty(ivr_ICEFeatureData.getConfig().getParamValue(Constants.CUI_CI_FundTransferIntra_Succ_ErrorCode))){
				if(logger.isDebugEnabled()){WriteLog.write(WriteLog.DEBUG,sessionID, "Success error code received from the Feature ");}
				hostErrorCodeList = (String)ivr_ICEFeatureData.getConfig().getParamValue(Constants.CUI_CI_FundTransferIntra_Succ_ErrorCode); 
			}
			else{
				if(logger.isDebugEnabled()){WriteLog.write(WriteLog.DEBUG,sessionID, "Success error code received from the Global ");}
				hostErrorCodeList = (String)ivr_ICEGlobalConfig.getConfig().getParamValue(Constants.CUI_CI_FundTransferIntra_Succ_ErrorCode);
			}
			...
			if(Constants.WS_SUCCESS_CODE.equals(code)){
				if(!util.isNullOrEmpty(response)){
					...
					if(!util.isNullOrEmpty(xferRecType)){
						if(logger.isDebugEnabled()){WriteLog.write(WriteLog.DEBUG,sessionID, "## RESONSE FIELD ## Xfer ID is "+ xferRecType.getXferId());}
			...
			if(logger.isDebugEnabled()){WriteLog.writeError(WriteLog.ERROR, pe,  "There was an error at FundTransferIntraDAOImpl.getFTInterPaymentUpdHostRes() "	+ pe.getMessage());}
			throw new DaoException(pe);
		}catch(Exception e){
			if(logger.isDebugEnabled()){WriteLog.writeError(WriteLog.ERROR, e,  "There was an error at FundTransferIntraDAOImpl.getFTInterPaymentUpdHostRes() "	+ e.getMessage());}
			throw new DaoException(e);
		}

		//if(logger.isInfoEnabled()){WriteLog.write(WriteLog.INFO, "*** EXIT - DISPACHING WS RES ***: FundTransferIntraDAOImpl.getFTInterPaymentUpdHostRes()");}
		return beanResponse;
	}

//	@Override
//	public UpdateFTIntraPayment_HostRes getFTTWBMPaymentUpdHostRes(
//			CallInfo callInfo, String xferId, String benfCode,
//			BigDecimal debitAmt, BigDecimal creditAmt, String acctID,
//			String destAcctID, String serviceProviderID, String utilityID,
//			String billID, String contractID, String requestType) throws DaoException {
