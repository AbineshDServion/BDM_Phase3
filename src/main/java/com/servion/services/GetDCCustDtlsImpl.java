...
public class GetDCCustDtlsImpl implements IGetDCCustDtls{

	private static Logger logger = LoggerObject.getLogger();
	
	private GetDCCustDtlsDAO getDCCustDtlsDAO; 
	private MessageSource messageSource;
	
	

	public GetDCCustDtlsDAO getGetDCCustDtlsDAO() {
		return getDCCustDtlsDAO;
	}

	public void setGetDCCustDtlsDAO(GetDCCustDtlsDAO getDCCustDtlsDAO) {
		this.getDCCustDtlsDAO = getDCCustDtlsDAO;
	}
	...
	@Override
	public String getGetDCCustDtls(CallInfo callInfo) throws ServiceException {
		...
		if(logger.isInfoEnabled()){WriteLog.write(WriteLog.INFO,session_ID_, "ENTER: GetDCCustDtlsImpl.getGetDCCustDtls()");}
		...
		try{
			...
			String requestType = util.isNullOrEmpty(ivr_ICEFeatureData.getConfig().getParamValue(Constants.CUI_GetDCCustDtls_REQUESTTYPE ))? null : (String)ivr_ICEFeatureData.getConfig().getParamValue(Constants.CUI_GetDCCustDtls_REQUESTTYPE);
			if(logger.isDebugEnabled()){WriteLog.write(WriteLog.DEBUG,session_ID_, "requestType configured is " + requestType);}

			
			GetDCCustDtls_HostRes getDCCustDtls_HostRes = getDCCustDtlsDAO.getGetDCCustDtlsHostRes(callInfo, cardNo, requestType);


			if(logger.isDebugEnabled()){WriteLog.write(WriteLog.DEBUG,session_ID_, "getDCCustDtls_HostRes Object is :"+ getDCCustDtls_HostRes);}
			callInfo.setDCCustDtls_HostRes(getDCCustDtls_HostRes);

			code = getDCCustDtls_HostRes.getErrorCode();

			/*
			 * For Reporting Start
			 */
			
			String hostEndTime = getDCCustDtls_HostRes.getHostEndTime();
			...
			String hostResCode = getDCCustDtls_HostRes.getHostResponseCode();
			...
			+ Constants.COMMA + Constants.HOST_INPUT_PARAM_ESBERRORDESC +Constants.EQUALTO +(util.isNullOrEmpty(getDCCustDtls_HostRes.getErrorDesc()) ?"NA" :getDCCustDtls_HostRes.getErrorDesc());
			...
			if(Constants.WS_SUCCESS_CODE.equalsIgnoreCase(code)){
				...
				if(logger.isDebugEnabled()){WriteLog.write(WriteLog.DEBUG,session_ID_, "The original response code of host access is " + getDCCustDtls_HostRes.getHostResponseCode());}

				util.setHostErrorCodePhrase(callInfo, Constants.HOST_METHOD_GetDCCustDtls, getDCCustDtls_HostRes.getHostResponseCode());
