...
public class FundsTransferThirdPartyOutsideBMImpl implements IFundsTransferThirdPartyOutsideBM{
	...
	@Override
	public String getFTOutsideBMBeneficiaryList(CallInfo callInfo)
...
//		if(logger.isInfoEnabled()){WriteLog.write(WriteLog.INFO,session_ID_, "ENTER: FundsTransferThirdPartyOutsideBMImpl.getFTOutsideBMBeneficiaryList()");}   
		...
		try{
			...
			String paymentType = Constants.HOST_FT_PAYMENTTYPE_EXTERNALTHIRDPARTYTRANSFER;
			...
			FT_BeneficiaryDetailList_HostRes ft_BeneficiaryDetailList_HostRes = listBeneficiaryDAO.getFTTOBMBenfListHostRes(callInfo, customerId, paymentType, requestType);
			...
			WriteLog.writeError(WriteLog.ERROR, e,  "There was an error at FundsTransferThirdPartyOutsideBMImpl.getFTOutsideBMBeneficiaryList() "+ e.getMessage());
	...
	public String getFTOutsideBMBeneficiaryDetails(CallInfo callInfo) throws ServiceException{
		String session_ID_ = "";try{session_ID_ = (String)callInfo.getField(Field.SESSIONID);logger = (Logger)callInfo.getField(Field.LOGGER);WriteLog.loggerInit(logger, session_ID_);}catch(Exception e){}
		if(logger.isInfoEnabled()){WriteLog.write(WriteLog.INFO,session_ID_, "ENTER: FundsTransferThirdPartyOutsideBMImpl.getFTOutsideBMBeneficiaryDetails()");}
		String code = Constants.EMPTY_STRING;
		try{
			...
			FT_BenfPayeeDetails_HostRes ft_BenfPayeeDetails_HostRes = beneficiaryDtlsInquiryDAO.getFTTOBMBenfDelsHostRes(callInfo, beneficiaryIdList, requestType);
			...
			if(logger.isInfoEnabled()){WriteLog.write(WriteLog.INFO,session_ID_, "EXIT: FundsTransferThirdPartyOutsideBMImpl.getFTOutsideBMBeneficiaryDetails()");}
		}catch(Exception e){
			WriteLog.writeError(WriteLog.ERROR, e,  "There was an error at FundsTransferThirdPartyOutsideBMImpl.getFTOutsideBMBeneficiaryDetails() "+ e.getMessage());
			throw new ServiceException(e);
		}
		return code;
	...
	@Override
	public String getFTOutsideBMBeneficiaryPhrases(CallInfo callInfo)throws ServiceException{
		
		String session_ID_ = "";try{session_ID_ = (String)callInfo.getField(Field.SESSIONID);logger = (Logger)callInfo.getField(Field.LOGGER);WriteLog.loggerInit(logger, session_ID_);}catch(Exception e){}
		if(logger.isInfoEnabled()){WriteLog.write(WriteLog.INFO,session_ID_, "ENTER: FundsTransferThirdPartyOutsideBMImpl.getFTOutsideBMBeneficiaryPhrases()");}
		...
		try{
			...
			String menuID = MenuIDMap.getMenuID("FUNDTRANSFER_BENEFICIARY_OUTSIDE_BM");
			//String anncID = AnncIDMap.getAnncID("");
			String featureID = FeatureIDMap.getFeatureID("Funds_Transfer_Third_Party_outside_BM");
	...
	public String OLD_getFTOutsideBMBeneficiaryPhrases(CallInfo callInfo)throws ServiceException{
		...
		try{
			...
			if(!util.isNullOrEmpty(callInfo.getFT_BeneficiaryDetailList_HostRes())){
				if(!util.isNullOrEmpty(callInfo.getFT_BeneficiaryDetailList_HostRes().getBeneficiaryIdList())){
					noOfExistingBenef = callInfo.getFT_BeneficiaryDetailList_HostRes().getBeneficiaryIdList().size();
					...
					if(noOfExistingBenef == 1 && (callInfo.getFT_BeneficiaryDetailList_HostRes().getBeneficiaryIdList().get(0) == null ||  
							callInfo.getFT_BeneficiaryDetailList_HostRes().getBeneficiaryIdList().get(0) == Constants.EMPTY)){
						noOfExistingBenef = Constants.GL_ZERO;
					}
				}
			}
			...
			if(noOfExistingBenef == Constants.GL_ONE){
				...
				String beneficiaryDetlHostCode = getFTOutsideBMBeneficiaryDetails(callInfo);
				...
				if(callInfo.getFT_BeneficiaryDetailList_HostRes()!=null){
					if(callInfo.getFT_BeneficiaryDetailList_HostRes().getBeneficiaryIdList() != null){
						beneficiaryID = callInfo.getFT_BeneficiaryDetailList_HostRes().getBeneficiaryIdList().get(Constants.GL_ZERO);		
					}
				}
				
						
						
				FT_BeneficiaryDetails ft_BeneficiaryDetails =	beneficiaryDetailMap.get(beneficiaryID);
				if(logger.isDebugEnabled()){WriteLog.write(WriteLog.DEBUG,session_ID_, "The Single account present is "  + ft_BeneficiaryDetails.getBenefAccountNo());}
				...
				if(isToAddNewBeneficiary){
					...
					dynamicValueArray.add(ft_BeneficiaryDetails.getBenefAccountNo());
					dynamicValueArray.add(DynaPhraseConstants.PHRASE_PRESS_2);
					grammar = Constants.FT_ADD_NEW_BENEFICIARY + Constants.COMMA + Constants.FT_ADD_EXISTING_BENEFICIARY;
				...
				callInfo.setField(Field.DESTNO, ft_BeneficiaryDetails.getBenefAccountNo());
				...
				if(isToAddNewBeneficiary){
					...
					throw new ServiceException("There is no any registerd beneficiary and add beneficiary option is there");
//					dynamicValueArray.add(DynaPhraseConstants.Funds_Transfer_1002);
//					dynamicValueArray.add(DynaPhraseConstants.PHRASE_PRESS_1);
//					dynamicValueArray.add(Constants.NA);
//					dynamicValueArray.add(Constants.NA);
...
//					grammar = Constants.FT_ADD_NEW_BENEFICIARY;
			...
			//Need to handle if we want to append pipeseperator sign
			//No Need
			if(logger.isInfoEnabled()){WriteLog.write(WriteLog.INFO,session_ID_, "EXIT: FundsTransferThirdPartyOutsideBMImpl.getFTOutsideBMBeneficiaryPhrases()");}

		}catch(Exception e){
			WriteLog.write(WriteLog.INFO, session_ID_,"ERROR: FundsTransferThirdPartyOutsideBMImpl.getFTOutsideBMBeneficiaryPhrases()");
			throw new ServiceException(e);
		}
		return finalResult;
	}
	...
	@Override
	public String getFTOutsideBMBeneficiaryAccNoPhrases(CallInfo callInfo)
			throws ServiceException {
		
		String session_ID_ = "";try{session_ID_ = (String)callInfo.getField(Field.SESSIONID);logger = (Logger)callInfo.getField(Field.LOGGER);WriteLog.loggerInit(logger, session_ID_);}catch(Exception e){}
		if(logger.isInfoEnabled()){WriteLog.write(WriteLog.INFO,session_ID_, "ENTER: FundsTransferThirdPartyWithinBMImpl.getFTOutsideBMBeneficiaryAccNoPhrases()");}
		String str_GetMessage, finalResult;
...
//			if(logger.isDebugEnabled()){WriteLog.write(WriteLog.DEBUG, "Selected service provider is  "+selectedServiceProvider);}
			...
			/**
			 * Following are the changes done on 29-Nov-2014
			 */
			...
			String menuID = MenuIDMap.getMenuID("FUNDTRANSFER_BENEFICIARY_ACCOUNT_SELECTION_OUTSIDE_BM");
			//String anncID = AnncIDMap.getAnncID("");
			String featureID = FeatureIDMap.getFeatureID("Funds_Transfer_Third_Party_outside_BM");
			String combinedKey = languageKey+Constants.UNDERSCORE+featureID+Constants.UNDERSCORE+menuID;
...
//			}
			...
			if(logger.isInfoEnabled()){WriteLog.write(WriteLog.INFO,session_ID_, "EXIT: FundsTransferThirdPartyWithinBMImpl.getFTOutsideBMBeneficiaryAccNoPhrases()");}
