...
public class UtilityBillPaymentElectricityImpl implements IUtilityBillPaymentElectricity{
	...
	@Override
	public String getElectricityBillAccountNumberMenuPhrases(CallInfo callInfo)
		...
		try{
			...
			String beneficiaryDetlHostCode = getElectricityPayeeDetails(callInfo);
			...
			for(int count=Constants.GL_ZERO;count<beneficiaryDetailsList.size();count++){
				...
				if(!util.isNullOrEmpty(temp_benefDetail)){
					...
					if(logger.isDebugEnabled()){WriteLog.write(WriteLog.DEBUG,session_ID_, "The electricity account number is"+temp_Str);}
			...
			String menuID = MenuIDMap.getMenuID("ELECTRICITY_NUMBER_SELECTION");
...
//					//END Vinoth
	...
	@Override
	public String getElectricityBillServiceProvidersPhrases(CallInfo callInfo)
			throws ServiceException {
		
		String session_ID_ = "";try{session_ID_ = (String)callInfo.getField(Field.SESSIONID);logger = (Logger)callInfo.getField(Field.LOGGER);WriteLog.loggerInit(logger, session_ID_);}catch(Exception e){}
		if(logger.isInfoEnabled()){WriteLog.write(WriteLog.INFO,session_ID_, "ENTER: UtilityBillPaymentElectricityImpl.getElectricityBillServiceProvidersPhrases()");}
		String str_GetMessage, finalResult;
		...
		try{
			...
			if(logger.isDebugEnabled()){WriteLog.write(WriteLog.DEBUG,session_ID_, "Retrieved utlity code list from host is "  + utilityCodeListFrmHost);}
			...
			for(int i=0; i<strHostArrayList.length;i++){
				...
				if(utilityCodeListFrmHost.contains(strUtilityCode)){
					...
					if(util.isNullOrEmpty(strServiceProviderList)){
						...
						throw new ServiceException("Configured service provider code at UI is null / empty");
					}
					...
					if(util.isNullOrEmpty(serviceProviderCodeListFromHost)){
						...
						throw new ServiceException("Configured service provider List from host is null / empty");
					}
...
//			 
			...
			String menuID = MenuIDMap.getMenuID("ELECTRICITY_SERVICEPROVIDER_SELECTION");
			//String anncID = AnncIDMap.getAnncID("");
			String featureID = FeatureIDMap.getFeatureID("Utility_Bill_Payment_Electricity");
			String combinedKey = languageKey+Constants.UNDERSCORE+featureID+Constants.UNDERSCORE+menuID;
...
//				}
			...
			if(logger.isInfoEnabled()){WriteLog.write(WriteLog.INFO,session_ID_, "EXIT:UtilityBillPaymentElectricityImpl.getElectricityBillServiceProvidersPhrases()");}
			
		}catch(Exception e){
			if(logger.isDebugEnabled()){WriteLog.writeError(WriteLog.ERROR, e,  "There was an error at UtilityBillPaymentElectricityImpl.getElectricityBillServiceProvidersPhrases() "	+ e.getMessage());}
			throw new ServiceException(e);
			}
		return finalResult;
	}
	...
	@Override
	public String getElectricityExstOrAddBenfPhrases(CallInfo callInfo)
			throws ServiceException {
		String session_ID_ = "";try{session_ID_ = (String)callInfo.getField(Field.SESSIONID);logger = (Logger)callInfo.getField(Field.LOGGER);WriteLog.loggerInit(logger, session_ID_);}catch(Exception e){}
		if(logger.isInfoEnabled()){WriteLog.write(WriteLog.INFO,session_ID_, "ENTER: UtilityBillPaymentElectricityImpl.getElectricityExstOrAddBenfPhrases()");}
		String str_GetMessage, finalResult;
...
//					if(callInfo.getUtility_BeneficiaryDetailList_HostRes().getBeneficiaryIdList().size() > Constants.GL_ZERO){
...
//			
			...
			//Need to handle if we want to append pipeseperator sign
			//No Need
			if(logger.isInfoEnabled()){WriteLog.write(WriteLog.INFO,session_ID_, "EXIT: UtilityBillPaymentElectricityImpl.getElectricityExstOrAddBenfPhrases()");}
			
		}catch(Exception e){
			if(logger.isDebugEnabled()){WriteLog.writeError(WriteLog.ERROR, e,  "There was an error at UtilityBillPaymentElectricityImpl.getElectricityExstOrAddBenfPhrases() "	+ e.getMessage());}
			throw new ServiceException(e);
			}
		return finalResult;
	}
	...
	@Override
	public String getElectricityPayeeList(CallInfo callInfo)
			throws ServiceException {
		String session_ID_ = "";try{session_ID_ = (String)callInfo.getField(Field.SESSIONID);logger = (Logger)callInfo.getField(Field.LOGGER);WriteLog.loggerInit(logger, session_ID_);}catch(Exception e){}
		if(logger.isInfoEnabled()){WriteLog.write(WriteLog.INFO,session_ID_, "ENTER: UtilityBillPaymentElectricityImpl.getElectricityPayeeList()");}
...
ICEFeatureData iceFeatureData = util.isNullOrEmpty(callInfo.getICEFeatureData())? null : (ICEFeatureData)callInfo.getICEFeatureData();
			...
			if(logger.isInfoEnabled()){WriteLog.write(WriteLog.INFO,session_ID_, "EXIT: UtilityBillPaymentElectricityImpl.getElectricityPayeeList()");}
		}catch(Exception e){
			WriteLog.writeError(WriteLog.ERROR, e,  "There was an error at UtilityBillPaymentElectricityImpl.getElectricityPayeeList() "+ e.getMessage());
			throw new ServiceException(e);
		}
		return code;		
	...
	@Override
	public String getElectricityPayeeDetails(CallInfo callInfo)
			throws ServiceException {
		String session_ID_ = "";try{session_ID_ = (String)callInfo.getField(Field.SESSIONID);logger = (Logger)callInfo.getField(Field.LOGGER);WriteLog.loggerInit(logger, session_ID_);}catch(Exception e){}
		if(logger.isInfoEnabled()){WriteLog.write(WriteLog.INFO,session_ID_, "ENTER: UtilityBillPaymentElectricityImpl.getElectricityPayeeDetails()");}
		String code = Constants.EMPTY_STRING;
		try{
...
//			}
			...
			Utility_BenfPayeeDetails_HostRes utility_BenfPayeeDetails_HostRes = beneficiaryDtlsInquiryDAO.getElecBillBenfDelsHostRes(callInfo, beneficiaryIdList, requestType);
			...
			if(logger.isInfoEnabled()){WriteLog.write(WriteLog.INFO,session_ID_, "EXIT: UtilityBillPaymentElectricityImpl.getElectricityPayeeDetails()");}
		}catch(Exception e){
			WriteLog.writeError(WriteLog.ERROR, e,  "There was an error at UtilityBillPaymentElectricityImpl.getElectricityPayeeDetails() "+ e.getMessage());
			throw new ServiceException(e);
		}
		return code;
	...
	@Override
	public String getElectricityBalanceDetails(CallInfo callInfo)
			throws ServiceException {
		String session_ID_ = "";try{session_ID_ = (String)callInfo.getField(Field.SESSIONID);logger = (Logger)callInfo.getField(Field.LOGGER);WriteLog.loggerInit(logger, session_ID_);}catch(Exception e){}
		if(logger.isInfoEnabled()){WriteLog.write(WriteLog.INFO, session_ID_,"ENTER: UtilityBillPaymentElectricityImpl.getElectricityBalanceDetails()");}
		...
		String electricityType = Constants.EMPTY_STRING;
		try{
			...
			if(isForOIFCService){
				...
				electricityType = util.isNullOrEmpty(iceFeatureData.getConfig().getParamValue(Constants.CUI_UTILITYOFICBALANCE_ELECTRICITYTYPE))? null : (String)iceFeatureData.getConfig().getParamValue(Constants.CUI_UTILITYOFICBALANCE_ELECTRICITYTYPE);
				if(logger.isDebugEnabled()){WriteLog.write(WriteLog.DEBUG, session_ID_, "Electricity type configured is " + electricityType);}
				
				UtilitySubscriberInfo_HostRes utilityBalanceDetails_HostRes = utilitySubscriberInfoDAO.getUtilitySubscriberInfo_HostRes(callInfo, requestType, providerType, serviceProviderCode, utilityCode, contractNo, isForOIFCService, electricityType);
				...
				if(Constants.WS_SUCCESS_CODE.equalsIgnoreCase(code)){
					...
					if( callInfo.getUtilitySubscriberInfo_HostRes() != null &&  callInfo.getUtilitySubscriberInfo_HostRes().getDueBalance()!=null){
						...
						callInfo.setField(Field.ELECTRICITY_TYPE, electricityType);
						callInfo.setField(Field.BILLER_CODE, callInfo.getUtilitySubscriberInfo_HostRes().getBillerCode());
					...
					if(logger.isDebugEnabled()){WriteLog.write(WriteLog.INFO, session_ID_, "EXIT :  UtilityBillPaymentElectricityImpl.getOFICBalanceDetails()");}
				}
				...
				if(Constants.WS_SUCCESS_CODE.equalsIgnoreCase(code)){
					...
					if( callInfo.getGetUtilityBillInfo_HostRes() != null && callInfo.getGetUtilityBillInfo_HostRes().getRecCount() > 0 
						...
						//if(logger.isDebugEnabled()){WriteLog.write(WriteLog.DEBUG, session_ID_,"Received the Bill Status" + callInfo.getGetUtilityBillInfo_HostRes().getBillStatus());}
					...
					if(logger.isDebugEnabled()){WriteLog.write(WriteLog.INFO, session_ID_, "EXIT :  UtilityBillPaymentElectricityImpl.getElectricityBalanceDetails()");}
				}
			...
			WriteLog.writeError(WriteLog.ERROR, e,  "There was an error at   UtilityBillPaymentElectricityImpl.getElectricityBalanceDetails() "+ e.getMessage());
			throw new ServiceException(e);
		}
		return code;
	...
	@Override
  	public void isSelectedServProviderAOnline(CallInfo callInfo)
  			throws ServiceException {
  		String session_ID_ = "";try{session_ID_ = (String)callInfo.getField(Field.SESSIONID);logger = (Logger)callInfo.getField(Field.LOGGER);WriteLog.loggerInit(logger, session_ID_);}catch(Exception e){}
  		if(logger.isInfoEnabled()){WriteLog.write(WriteLog.INFO,session_ID_,  "ENTER: UtilityBillPaymentElectricityImpl.isSelectedServProviderAOnline()");}
  		try{
			...
			if(!util.isNullOrEmpty(beneficiaryDetailsList)){
  			...
  			if(logger.isInfoEnabled()){WriteLog.write(WriteLog.INFO, session_ID_,"EXIT: UtilityBillPaymentElectricityImpl.isSelectedServProviderAOnline()");}
  		}catch(Exception e){
  			if(logger.isInfoEnabled()){WriteLog.write(WriteLog.ERROR,session_ID_, "ERROR: UtilityBillPaymentElectricityImpl.isSelectedServProviderAOnline()");}
  
  			if(logger.isDebugEnabled()){WriteLog.writeError(WriteLog.ERROR, e,  "There was an error at UtilityBillPaymentElectricityImpl.isSelectedServProviderAOnline() "+ e.getMessage());}
  			throw new ServiceException(e);
  		}
  	}
	...
	@Override
  	public String totalBillPaymentAmtMenuPhrases(CallInfo callInfo)
  			throws ServiceException {
  		String session_ID_ = "";try{session_ID_ = (String)callInfo.getField(Field.SESSIONID);logger = (Logger)callInfo.getField(Field.LOGGER);WriteLog.loggerInit(logger, session_ID_);}catch(Exception e){}
  		if(logger.isInfoEnabled()){WriteLog.write(WriteLog.INFO,session_ID_, "ENTER: UtilityBillPaymentElectricityImpl.totalBillPaymentAmtMenuPhrases()");}
  		String str_GetMessage, finalResult;
  		...
  		try{
  			...
  			String menuID = MenuIDMap.getMenuID("Electricity_OFIC_TOTALBILLMENU");
  			//String anncID = AnncIDMap.getAnncID("");
  			String featureID = FeatureIDMap.getFeatureID("UtilityBillPaymentElectricity");
		...
		//To have the property file grammar, need to call that util method here
 		...
 		finalResult = util.callDynaPhraseGeneration(dynamicPhraseKey, dynamicMessageValue, totalPrompt);
			...
			//Need to handle if we want to append pipeseperator sign
			//No Need
			if(logger.isInfoEnabled()){WriteLog.write(WriteLog.INFO,session_ID_, "EXIT: UtilityBillPaymentElectricityImpl.totalBillPaymentAmtMenuPhrases()");}

		}catch(Exception e){
			if(logger.isDebugEnabled()){WriteLog.writeError(WriteLog.ERROR, e,  "There was an error at UtilityBillPaymentElectricityImpl.totalBillPaymentAmtMenuPhrases() "	+ e.getMessage());}
			throw new ServiceException(e);
		}
		return finalResult;
	}
